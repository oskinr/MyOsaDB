...
class EditableTableWidget:
    def __init__(self, parent_frame, table_data, master_var):
        ...
        # Словарь для хранения BooleanVar строк. Ключом будет уникальный rowid.
        self.vars_for_saving = {}
        # Словарь для связи: значение из JSON <-> уникальный rowid
        self.json_value_to_uid = {}

        # Сохраняем ссылку на canvas, чтобы можно было обновить таблицу
        self.table_canvas = None 

        self._create_widgets()

    def _create_widgets(self):
        # --- 1. СОЗДАНИЕ ВИДЖЕТОВ И ФРЕЙМОВ ---
        # Удаляем старые виджеты, если метод update_data вызывается повторно
        for widget in self.parent_frame.winfo_children():
            widget.destroy()

        table_outer_frame = ttk.Frame(self.parent_frame)
        table_outer_frame.pack(fill="both", expand=True, pady=5)

        self.table_canvas = tk.Canvas(table_outer_frame)
        scrollbar = ttk.Scrollbar(table_outer_frame, orient="vertical", command=self.table_canvas.yview)
        scrollable_inner = ttk.Frame(self.table_canvas)

        scrollable_inner.bind('<Configure>', lambda e: [
            self.table_canvas.configure(scrollregion=self.table_canvas.bbox("all")),
            self.table_canvas.configure(width=e.width)
        ])

        self.table_canvas.create_window((0, 0), window=scrollable_inner, anchor="nw")
        self.table_canvas.configure(yscrollcommand=scrollbar.set)

        self.table_canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")

        # Очистка словарей при повторном вызове
        ...
        for row_idx, row in enumerate(self.table_data):
            ...
            # --- БЛОК А: ПЕРВАЯ СТРОКА (ШАПКА И "ВЫБРАТЬ ВСЁ") ---
            if row_idx == 0:
                # Функция для "Выбрать всё"
                def on_master_check():
                    """Устанавливает состояние всех чекбоксов в соответствии с главным."""
                ...
                for col_num, key in enumerate(['col1', 'col2', 'col3'], start=1):
                    if key in row and row[key]:
                        ...
                        ).grid(row=0, column=col_num, sticky="we", padx=(5 if col_num>1 else 0, 5), pady=2)
                        scrollable_inner.grid_columnconfigure(col_num, weight=1)
                continue

            # --- БЛОК Б: ОСТАЛЬНЫЕ СТРОКИ (ДАННЫЕ) ---
            if value_to_find:
                ...
                # --- TRACE: Логика обратной связи (от строки к "Выбрать всё") ---
                # Фикс: используем default-аргумент current_var, чтобы передать текущее значение
                var.trace_add('write', lambda *args, 
                    m_var=self.master_var, 
                    save_dict=self.vars_for_saving,
                    current_var=var: 
                        # Проверяем все КРОМЕ текущей переменной
                ...
                for col_num, key in enumerate(['col1', 'col2', 'col3'], start=1):
                    ...
                    ttk.Label(scrollable_inner, text=cell_value, anchor="w").grid(
                        row=data_row_counter, column=col_num, sticky="we",
    ...
    def update_data(self, new_table_data):
        """
        Обновляет данные таблицы и перерисовывает виджеты.
        Просто вызывает метод отрисовки заново с новыми данными.
        """
        self.table_data = new_table_data
        self._create_widgets()
    def destroy(self):
            """Полностью удаляет виджет таблицы с экрана."""
            # Виджет table_outer_frame является дочерним для parent_frame
            # Найдем его и удалим
            for widget in self.parent_frame.winfo_children():
                if widget.winfo_class() == 'TFrame': # Ищем внешний фрейм таблицы
                    widget.destroy()
       ...
       # 3. Парсим JSON-текст в список Python
        ...
        for idx, item in enumerate(config_list):
            ...
            # --- 4. Таблицы (table) - УНИВЕРСАЛЬНЫЙ КОД ---
                ...
                # Создаём виджет таблицы и передаём ему фрейм и данные
                table_widget = EditableTableWidget(inner_frame, table_data, master_var)
                
                # Сохраняем данные для save_all_changes, используя ИДЕКС элемента как ключ
                widget_data[idx] = {  # <-- ИСПОЛЬЗУЕМ ИНДЕКС ВМЕСТО LABEL
                    "type": "table",
                    "vars": table_widget.get_vars(),
                    "value_to_uid_map": table_widget.get_value_to_uid_map(),
                    "original_data": table_data,
                    "label": label, # Сохраняем label для отображения, если нужно
                }
        ...
        def save_all_changes():
            # Проходим по всем элементам config_list с индексом
            for idx, item in enumerate(config_list):
                # 1. Пытаемся найти данные для этого элемента.
                # Сначала ищем по индексу (для таблиц), затем по label (для полей)
                data = widget_data.get(idx)
                
                # Если по индексу не нашли, ищем по label
                if data is None:
                    label = item.get("label")
                    if label:
                        data = widget_data.get(label)

                # Если данных для этого элемента нет, пропускаем его
                if not data:
                    continue 

                # 2. Теперь, когда data найден, обрабатываем его в зависимости от типа
                if data["type"] == "text_field":
                    new_value = data["var"].get()
                    # Обновляем значение в исходном JSON
                    if data["name"] and "text_element" in item:
                        item["text_element"]["value"] = new_value

                elif data["type"] == "combo_box":
                    new_text = data["var"].get()
                    # Сбрасываем все флаги selected и ставим его только у выбранного значения
                    ...
                    # Логика для таблиц остается прежней, так как она уже работала
            ...
            # ... далее код записи в БД остается без изменений
...
# Главное окно
root = tk.Tk()
root.title("Работа с базой Smapp.3db v1.2")
